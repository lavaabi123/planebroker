...
class CaptainsModel extends Model
{
    protected $DBGroup          = 'default';
    protected $table            = 'captains_club_request';
    ...
    public function DataPaginations()
    {
        ...
        $paginateData = $this->select('captains_club_request.*');
        ...
        if (!empty($search)) {
            $this->builder()->groupStart()
                ->like('captains_club_request.from_name', clean_str($search))   
                ->orLike('captains_club_request.from_email', clean_str($search))              
                ->groupEnd();
        }

        $status = trim($this->request->getGet('status') ?? '');
        if ($status != null && ($status == 1 || $status == 0)) {
            $this->builder()->where('captains_club_request.status', clean_number($status));
        }

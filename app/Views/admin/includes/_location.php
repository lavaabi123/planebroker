<?php if (!empty($countries)) : ?>
    <div class="row">
        <div id="get_county_container" class="col-12 col-sm-3 m-b-15">
            <select id="select_countries" name="county_id" class="select2 form-control" onchange="get_states(this.value, '<?php echo $map; ?>');" required>
                <option value=""><?php echo trans('county'); ?></option>
                <?php foreach ($this->counties as $item) :
                    if (!empty($county_id)) : ?>
                        <option value="<?php echo $item->id; ?>" <?php echo ($item->id == $county_id) ? 'selected' : ''; ?>><?php echo html_escape($item->name); ?></option>
                    <?php else : ?>

...
<?= $this->section('content') ?>
    ...
    <div class="content-header px-0">
		...
		<div class='row'>
			...
			<h6 class='text-lg'><b><i class='fa fa-trophy'></i> Purchase <?php echo $plan_detail[0]->name; ?> Plan for $<?php echo str_replace('.00', '', $plan_detail[0]->price); ?></b></h6>
			...
			<div class='row'>
				<div class='col-12 col-sm-6'>
					...
					<div class="carddetail">
						<p class="text-primary fw-medium">Use user's saved cards</p>
						<?php 
						if(!empty($cards)){
							foreach ( $cards->data as $s => $source ) { ?>
								<div class="card mb-2 rounded-5 p-0">
					...
					<div class="card mb-2 rounded-5 p-0">
						...
						<input type="hidden" name="from_end" value="admin" />
						...
						<input type="hidden" name="payment_type" value="<?php echo !empty($_GET['payment_type'])?$_GET['payment_type']:'stripe'; ?>" />
					<div class="s-hide" style="display:<?php echo !empty($cards) ? 'none' : ''; ?>">
					<?php if(!empty($cards)){ ?>					
					<p>Please enter User's new credit card information below</p>
					<?php }else{ ?>	
					<p>Please enter User's credit card information below</p>
					<?php } ?>	
						  <div class="form-row">
							<label for="card-element">Credit or debit card</label>
							<div id="card-element" style="border-radius:20px; width: 100%; border: 1px solid #eee; padding: 10px;margin: 10px;background-color:#ffffff;">
			  ...
			  <div class='text-sm'><i>User's first charge will take place on <?php echo date('F jS, Y', strtotime("+1 month")); ?>. You will be billed every <?= $plan_detail[0]->no_of_weeks ?> weeks. Cancel anytime by going to the subscriptions page on this dashboard.</i></div>
			  <?php }else{ ?>
			  <div class='text-sm'><i>User will be billed every <?= $plan_detail[0]->no_of_weeks ?> weeks. Cancel anytime by going to the subscriptions page on this dashboard.</i></div>
			  <?php } ?>
			  
						  <br>
						  <h4>Total: <span class="">$<?php echo str_replace('.00', '', $plan_detail[0]->price); ?></span></h4>
						  <br>
					...
					<p>(or)<p>
					<input type='button' onclick="change_plan('<?php echo $_GET['sale_id']; ?>','<?php echo $plan_detail[0]->id; ?>','<?php echo $type; ?>','<?php echo $_GET['user_id']; ?>')" value='Upgrade Plan without Payment' class='btn card-submit-btn'>
					<!--
					  ...
					  paypal.Buttons({
						  ...
						  onApprove: function(data, actions) {
							...
							$.ajax({
								...
								type: 'POST',
								dataType: 'JSON',
								data: {response:data,type:'<?php echo $type; ?>',plan_id:'<?php echo $plan_detail[0]->id; ?>',payment_type:'<?php echo !empty($_GET['payment_type'])?$_GET['payment_type']:''; ?>',sale_id:'<?php echo !empty($_GET['sale_id'])?$_GET['sale_id']:''; ?>',user_id:'<?php echo !empty($_GET['user_id']) ? $_GET['user_id']:''; ?>'},
								error: function() {
									Swal.fire({
								...
								success: function(res) {
									if(res.type == 'success'){
										Swal.fire({
											...
											//window.location.href = "<?php echo admin_url(); ?>listings/add?user_id=<?php echo !empty($_GET['user_id']) ? $_GET['user_id']:''; ?>&sale_id="+res.sale_id+"&payment_type=paypal&proceed=listing&plan_id=<?php echo $plan_detail[0]->id; ?>";
											window.location.href = "<?php echo admin_url(); ?>listings/sales";
										})
									}else{
										Swal.fire({
											text: res.message,
				...
				<div class='col-12 col-sm-6'>
					<table class="table table-striped include-plan" cellspacing="0" width="100%">
						<thead>
...
<script>
function change_plan(sale_id,plan_id,ptype,user_id){
	$.confirm({
			title: 'Confirm Updating Plan',
			content: 'Are you sure you want to change to this plan?',
			buttons: {
				confirm: function () {
					$.ajax({
						url: '<?php echo admin_url(); ?>listings/change_plan_post',
						data: {sale_id:sale_id,plan_id:plan_id,ptype:ptype,user_id:user_id},
						type: 'POST',
						dataType: 'HTML',

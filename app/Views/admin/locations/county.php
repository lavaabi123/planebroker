						...
						<div class="row filter_Sec table-filter-container align-items-end py-3">
							<div class="col-sm-12 d-flex align-items-end flex-column flex-sm-row">
								...
								<div class="item-table-filter">
									...
									<a href="javascript:void(0)" class="btn small bg-primary" data-toggle="modal" data-target="#modal-county"><i class="fa fa-plus pr-2"></i><?php echo trans("add"); ?></a>
								</div>
                                                        ...
                                                        <td>
                                                            <div class="dropdown btn-group">
                                                                ...
                                                                <div class="dropdown-menu dropdown-menu-animated">
                                                                    <?php if ($item->status == 1) : ?>
                                                                        <a class="dropdown-item" href="javascript:void(0)" data-toggle="modal" data-target="#modal-county" onclick="$('form').attr('action', '<?php echo admin_url() ."locations/county/saved-county-post";?>'); $('#modal-modalLabel').text('<?php echo trans('edit'); ?>'); $('#modal_id').val('<?php echo html_escape($item->id); ?>'); $('#modal_name').val('<?php echo html_escape($item->name); ?>'); $('#status_1').prop('checked', true);"><?php echo trans('edit'); ?></a>
                                                                    <?php else : ?>
                                                                        <a class="dropdown-item" href="javascript:void(0)" data-toggle="modal" data-target="#modal-county" onclick="$('form').attr('action', '<?php echo admin_url() ."locations/county/saved-county-post";?>'); $('#modal-modalLabel').text('<?php echo trans('edit'); ?>'); $('#modal_id').val('<?php echo html_escape($item->id); ?>'); $('#modal_name').val('<?php echo html_escape($item->name); ?>'); $('#status_2').prop('checked', true);"><?php echo trans('edit'); ?></a>
                                                                    <?php endif; ?>
                                                                    <div class=" dropdown-divider">
                                                                    </div>
                                                                    <a class="dropdown-item" href="javascript:void(0)" onclick="delete_item('/admin/locations/county/delete-county-post','<?php echo $item->id; ?>','<?php echo trans('confirm_delete'); ?>');"><?php echo trans('delete'); ?></a>
                                                                </div>
                                                            </div>
		...
		if(ids.length > 0) {
			Swal.fire({
				...
				if (response.value) {
					$.ajax({
						type: "POST",
						url: "<?php echo admin_url(); ?>"+"locations/county/bulk-delete-county-post",

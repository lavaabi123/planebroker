                              ...
                              <!--</div>--><!-- premium-plan-block -->                                  
                                    ...
                                    <div class="row">
										...
										if(!empty($dynamic_fields)){
											...
											<h5><?= esc($groupName) ?></h5>
											<?php $dynamic_fields_values = !empty($product_detail['dynamic_fields']) ? json_decode($product_detail['dynamic_fields'],true) : array();
											...
											foreach($dynamic_field as $field){
												...
												echo '<div class="form-group pr-2 d_fields" data-category="'.$field->category_ids.'" data-subcategory="'.$field->subcategory_ids.'">
														...
														if($field->field_type == 'Text'){
															echo '<input type="text" name="dynamic_fields['.$field->id.']" class="form-control" placeholder="'.$field->name.'" value="'. (!empty($dynamic_fields_values[$field->id]) ? $dynamic_fields_values[$field->id] : '').'" '.$req_op_text.'>';
														}else if($field->field_type == 'Number'){
															echo '<input type="number" name="dynamic_fields['.$field->id.']" class="form-control" placeholder="'.$field->name.'" value="'. (!empty($dynamic_fields_values[$field->id]) ? $dynamic_fields_values[$field->id] : '').'" '.$req_op_text.'>';
														}else if($field->field_type == 'Textarea'){
															echo '<textarea name="dynamic_fields['.$field->id.']" class="form-control" placeholder="'.$field->name.'" '.$req_op_text.'>'. (!empty($dynamic_fields_values[$field->id]) ? $dynamic_fields_values[$field->id] : '').'</textarea>';
															...
															if (!empty($decoded_option) && count($decoded_option) > 0) {
																...
																foreach($decoded_option as $oi => $option){
																	echo '<div class="col-sm-4 col-xs-12 col-option d-flex"><input type="checkbox" name="dynamic_fields['.$field->id.'][]" id="status_'.$oi.'" class="" placeholder="" value="'.$option.'" '. ((!empty($dynamic_fields_values[$field->id]) && in_array($option, $dynamic_fields_values[$field->id]) ) ? 'checked' : '').'  '.$req_op_text.'><label for="status_'.$oi.'" class="option-label">'.$option.'</label></div>';
																}	
																echo '</div>';															
															}else{
															...
															$option_html = '';
															if (!empty($decoded_option) && count($decoded_option) > 0) {
																echo '<select class="form-control" name="dynamic_fields['.$field->id.']"  '.$req_op_text.'><option value="">--'.$field->name.'--</option>';
																foreach($decoded_option as $oi => $option){
															...
															echo '<input type="file" name="dynamic_fields['.$field->id.']" class="form-control" placeholder="'.$field->name.'" value="'. (!empty($dynamic_fields_values[$field->id]) ? $dynamic_fields_values[$field->id] : '').'" '.$req_op_text.'>';
														}
													echo '</div>';
											}
											echo '</div>						
									...
									/*if(!empty($dynamic_fields)){
										...
										foreach($dynamic_fields as $field){
											echo '<div class="form-group mb-3 pr-2 d_fields"  data-category="" data-subcategory="">

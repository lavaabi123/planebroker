                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-6">
                                            <div class="form-group mb-3">
                                                ...
                                                <input type="text" name="last_name" id="last_name" class="form-control auth-form-input" placeholder="<?php echo trans("Last Name"); ?>" value="<?php echo html_escape($user_detail->last_name); ?>">
                                            </div>
                                        </div>
                                        <div class="col-6">
                                            <div class="form-group mb-3">
                                        ...
                                        <div class="col-6" id="userMailWrapper" style="display:none;">
                                            <div class="form-group">
												...
												<select name="admin_email_notify" class="form-control">
													<option value="0" <?php echo ($user_detail->admin_email_notify == 0) ? 'selected' : '' ; ?>>No</option>
													<option value="1" <?php echo ($user_detail->admin_email_notify == 1) ? 'selected' : '' ; ?>>Yes</option>
												</select>
                                            </div>
                                        </div>	
...
(function($){
  ...
  $(async function(){
    ...
    $('#cropper-confirm').on('click', function(){
      ...
      if (!canvas){ Swal.fire('Could not crop this image.', 'warning'); return; }

      canvas.toBlob(function(blob){
        if (!blob){ Swal.fire('Could not prepare the cropped image.', 'warning'); return; }
          ...
          success: function (response) {
            ...
            if (response?.uploaded == 1) {
              ...
              $.ajax({
                ...
                type: 'POST',
                dataType: 'json',
                data: addCsrf({ image: response.fileName,user_id:'<?php echo $user_detail->id; ?>' }),
                ...
                error: function(){ Swal.fire('Saved file, but failed to update profile. Please refresh.', 'warning'); }
              });
            } else {

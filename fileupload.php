...
function sanitizeFilename($filename) {
  ...
  // Remove special characters (keep only letters, numbers, underscore, hyphen)
...
if ($uploadId !== null && $chunkIndex !== null && $chunkTotal !== null && $chunkTotal > 1) {
  ...
  // Last chunk: assemble
  // === CHANGED: Sanitize the original filename ===
  $sanitizedOrigName = sanitizeFilename($origName);
  $finalName = rand() . '_' . time() . '_' . uniqid() . '_' . $sanitizedOrigName;
...
// === CHANGED: Sanitize the uploaded filename ===
$sanitizedUploadName = sanitizeFilename($_FILES['upload']['name']);
$filename = rand() . '_' . time() . '_' . uniqid() . '_' . $sanitizedUploadName;
